...
import os
#In this script, information is gathered from the RCSB PDB database using their API, the data is then checked for errors and compared to the pdbbind data for any inconsistensies
#All data and errors are then saved into a new file.
...
def get_info(pdb_id):
    ...
    sdf_file = f"PDBBind_processed/{pdb_id}/{pdb_id}_ligand.sdf"
    PDBBind_info_dict_location = "/data_output/PDBBind_info_dict.txt"

    with open(PDBBind_info_dict_location, "r") as file:
        index = 0
        PDBBind_info_dict = eval(file.read())
        try:
            PDBBind_for_pdb_id = PDBBind_info_dict[pdb_id]
        except KeyError:
            PDBBind_for_pdb_id = "Error: not in dict"
        for id in ligand_ids:
            if id == PDBBind_for_pdb_id:
                ligand_id_selected_index = index
                ligand_id_selected = id
                InChIKeys_selected = InChIKeys[index]
                SMILES_selected = SMILES[index]
            index += 1
        if not "InChIKeys_selected" in locals():
            InChIKeys_selected = ""
            SMILES_selected = ""
            ligand_id_selected = ""
            for length in length_list:
                if str(length) + "-mer" == PDBBind_for_pdb_id:
                    ligand_id_selected = "Protein with length " + str(length)
            if not "ligand_id_selected" in locals():
                ligand_id_selected = ""
    ...
    if os.path.isfile(sdf_file):
        atom_counts = count_atoms(sdf_file)
        formula_dicts_selected = []
        if atom_counts == defaultdict(int):
            formula_dicts_selected.append("Error: could not simulate")
        else:
            for formula in formula_dicts:
                if formula == atom_counts:
                    formula_dicts_selected.append(formula)
        ...
        formula_dicts_selected = ["Error: File Does not exist"]




    output = [pdb_id, [uniprot_ids, IPR_ids, Enzyme_class, polymer_ids, length_list], [InChIKeys_selected, formula_dicts_selected, ligand_id_selected, SMILES_selected]]
    return output

pdb_id_list_location = "timesplit_no_lig_overlap_train"
...
output_destination= "data_output/output_training_data.txt"
